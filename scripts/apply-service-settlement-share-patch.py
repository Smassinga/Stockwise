...
p = Path(settlements)
text = p.read_text()
old_filter = ".filter(order => !isCancelled(order.legacy_status) && order.workflow_status !== 'cancelled' && order.financial_anchor === 'legacy_order_link')"
if text.count(old_filter) != 2:
    raise SystemExit(f"{settlements}: expected two order-stage settlement filters, found {text.count(old_filter)}")
text = text.replace(
    old_filter,
    ".filter(order => !isCancelled(order.legacy_status) && order.workflow_status === 'approved' && order.financial_anchor === 'legacy_order_link')",
)
old_stale = """      } else if (message.includes('settlement_anchor_not_ready') || message.includes('settlement_anchor_not_found')) {
        toast.error(tt('settlements.anchorStale', 'This settlement anchor is no longer ready. Refresh the settlement workspace.'))
"""
new_stale = """      } else if (message.includes('settlement_anchor_not_ready') || message.includes('settlement_anchor_not_found')) {
        setActiveRow(null)
...
if text.count(old_stale) != 1:
    raise SystemExit(f"{settlements}: stale-anchor handler did not match")
p.write_text(text.replace(old_stale, new_stale, 1))

jobs = "src/pages/ServiceJobs.tsx"
replace_once(
    jobs,
...
marker = """      <PremiumSection title={tt('serviceJobs.supplierAllocations', 'Supplier allocations')} description={tt('serviceJobs.supplierAllocationHelp', 'Allocations assign existing approved Vendor Bill line cost; they do not create another payable.')}>
"""
...
if text.count(marker) != 1:
    raise SystemExit(f"{jobs}: supplier allocation insertion marker did not match")
