...
begin
  ...
  left join public.bins b
   ...
   and b."warehouseId"::text = r.warehouse_to_id::text
  where b.id is null
  order by r.row_no
  limit 1;
  ...
  left join public.stock_levels sl
   ...
   and sl.bin_id::text = buckets.bin_to_id::text;
  ...
  loop
    ...
    values (
      ...
      v_row.unit_cost,
      case
        when v_row.total_value > 0 then v_row.total_value
        else round(v_row.qty_base * v_row.unit_cost, 2)
      end,
      v_row.warehouse_to_id,
      v_row.bin_to_id,
      coalesce(v_row.notes, 'Stock inicial'),
      v_user,
      'ADJUST',
      null,
      null
