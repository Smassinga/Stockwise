-- OPS-1 production QA: preserve the authoritative report contract while
-- comparing legacy text audit/reference identifiers to UUID keys safely.
...
begin
  ...
  if v_code = 'performance' then
    ...
    left join public.sales_orders so on sm.ref_type='SO' and so.id::text=sm.ref_id
    left join public.purchase_orders po on sm.ref_type='PO' and po.id::text=sm.ref_id
    left join public.profiles p on p.id::text=sm.created_by
