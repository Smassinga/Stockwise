...
-- Fresh-company bootstrap must not reseed the global UOM catalogue.
-- UOM-INTEGRITY-1 made public.uoms a governed shared catalogue with
-- canonical semantic-identity enforcement. Re-seeding that catalogue during
-- tenant bootstrap now raises uom_equivalent_exists before ON CONFLICT can
-- reuse canonical rows, aborting the entire company-creation transaction.
...
comment on function public.create_company_and_bootstrap(text)
  is 'Create the authenticated user''s own company or return an existing active company. Pending invitations are left untouched and require explicit acceptance. Global UOM catalogue seeding is migration-managed and intentionally excluded from tenant bootstrap.';
