...
function log(...args: any[]) { if (DEBUG_LOG) console.log(...args); }
...
function textEmail(d: DigestPayload): string {
  ...
  if (!d.by_product.length) {
    ...
    for (const p of d.by_product.slice(0, 100)) {
      ...
      lines.push(
        `  • ${name} | Qty ${p.qty} | Rev ${currency(p.revenue)} | COGS ${currency(p.cogs)} | GP ${currency(p.gross_profit)} | GM ${p.gross_margin_pct}%`
      );
    }
  }
  lines.push("");
...
function htmlEmail(d: DigestPayload): string {
  ...
  const rows = d.by_product.map((p) => `
    <tr>
      <td style="padding:8px;border-bottom:1px solid #eee;">${p.item_label || p.item_name || p.item_id}</td>
      ...
      <td style="padding:8px;border-bottom:1px solid #eee;text-align:right;">${currency(p.cogs)}</td>
      <td style="padding:8px;border-bottom:1px solid #eee;text-align:right;">${currency(p.gross_profit)}</td>
      <td style="padding:8px;border-bottom:1px solid #eee;text-align:right;">${p.gross_margin_pct}%</td>
    </tr>
  `).join("");

  return `
  <div style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;max-width:720px;padding:16px;line-height:1.45;">
    <!-- preheader: hidden snippet shown by many clients -->
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;margin-bottom:16px;">
      <tr><td style="padding:10px;background:#f7f7f7;">Revenue</td><td style="padding:10px;background:#f7f7f7;text-align:right;">${currency(d.totals.revenue)}</td></tr>
      <tr><td style="padding:10px;">COGS</td><td style="padding:10px;text-align:right;">${currency(d.totals.cogs)}</td></tr>
      <tr><td style="padding:10px;">Gross Profit</td><td style="padding:10px;text-align:right;">${currency(d.totals.gross_profit)}</td></tr>
      <tr><td style="padding:10px;">Gross Margin</td><td style="padding:10px;text-align:right;">${d.totals.gross_margin_pct}%</td></tr>
    </table>

    <h3 style="margin:16px 0 8px 0;font-size:16px;">By Product</h3>
    <table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;">
      ...
      <tbody>
        ${rows || `<tr><td colspan="6" style="padding:12px;color:#666;">No product rows for this day.</td></tr>`}
      </tbody>
    </table>
...
async function sendViaSendGrid(to: string[], subject: string, html: string, text: string) {
  const body = {
    ...
    subject,
    content: [
      { type: "text/plain", value: text },          // plain alternative (deliverability)
...
function authorized(req: Request): boolean {
  ...
  const ok = hdr === DIGEST_HOOK_SECRET || bearer === DIGEST_HOOK_SECRET || (DEBUG_ACCEPT_QUERY_KEY && key === DIGEST_HOOK_SECRET);
  log("[auth]", { hdr: !!hdr, bearerLen: bearer.length, queryKey: !!key, ok });
  return ok;
}
...
serve(async (req: Request) => {
  try {
    ...
    // If you later add retries with next_attempt_at, this line already handles it.
    ...
    if (DRY_RUN) {
      ...
      await sendViaSendGrid(emails, subject, html, text);
    }
