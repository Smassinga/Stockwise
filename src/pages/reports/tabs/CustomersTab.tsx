...
export default function CustomersTab() {
  ...
  async function fetchRows(activeCompanyId = companyId) {
    ...
    try {
      ...
      if (customerId !== 'ALL') queryBuilder = queryBuilder.eq('customer_id', customerId)
      ...
      const filtered = !term
        ? data || []
        : (data || []).filter((row: any) => {
            const haystack = `${row.ref_type || ''} ${row.ref_no || ''} ${row.item_name || ''} ${row.item_sku || ''} ${row.customer_name || ''}`.toLowerCase()
  ...
  const totalCogs = useMemo(() => rows.reduce((sum, row) => sum + num(row.total_value), 0), [rows])
  const stamp = endDate.replace(/-/g, '')
  const exportBody: Row[] = [
    [tt('table.date', 'Date'), tt('table.ref', 'Ref'), tt('table.item', 'Item'), tt('table.qtyBase', 'Qty (base)'), `${tt('kpi.cogs.title', 'COGS')} (${displayCurrency})`, tt('table.notes', 'Notes')],
    ...rows.map((row) => [
  ...
  const onCSV = async () => {
    await downloadCSV(`customer_movements_${stamp}.csv`, [
      ...headerRows(ctx, tt('reports.customerMovements', 'Customer Movements')),
      ...formatRowsForCSV(exportBody, ctx, [4], [3]),
    ])
  }

  const onXLSX = async () => {
    await saveXLSX(`customer_movements_${stamp}.xlsx`, ctx, [
      { title: tt('reports.sheet.movements', 'Movements'), headerTitle: tt('reports.customerMovements', 'Customer Movements'), body: exportBody, moneyCols: [4], qtyCols: [3] },
    ])
  }
  ...
  return (
    <Card className="rounded-2xl border-border/80 shadow-sm">
      <CardHeader className="pb-3">
        <CardTitle>{tt('reports.customers.title', 'Customers')}</CardTitle>
        <p className="text-sm text-muted-foreground">{tt('reports.customerMovementsHelp', 'Review item-level movement and cost history by customer without leaving the reporting workspace.')}</p>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">{tt('reports.customerMovementCountHelp', 'Movement rows currently matching the selected customer and date filters.')}</p>
          </div>
          <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
            <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-muted-foreground">{tt('kpi.cogs.title', 'COGS')}</p>
            <div className="mt-2 text-lg font-semibold">{formatMoneyBase(totalCogs, baseCode)}</div>
            <p className="mt-1 text-xs text-muted-foreground">{tt('reports.customerCogsHelp', 'Base-currency cost carried by the filtered customer movement rows.')}</p>
          </div>
          <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
            ...
            <div className="mt-2 text-lg font-semibold">{customerId === 'ALL' ? customers.length : 1}</div>
            <p className="mt-1 text-xs text-muted-foreground">{tt('reports.customerCoverageReportHelp', 'Customer scope currently in view for this movement report.')}</p>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40">
                <tr className="text-left">
                  ...
                  <th className="px-3 py-2 text-right">{tt('kpi.cogs.title', 'COGS')}</th>
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  <tr key={row.id} className="border-t">
                    ...
                    <td className="px-3 py-3 text-right font-mono tabular-nums">{num(row.qty_base)}</td>
                    <td className="px-3 py-3 text-right font-mono tabular-nums">{formatMoneyBase(num(row.total_value), baseCode)}</td>
