  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as DatasetKey)} className="space-y-6">
        <TabsList className="h-auto w-full flex-wrap justify-start rounded-3xl border border-border/70 bg-background/80 p-2">
          {DATASETS.map((dataset) => (
            <TabsTrigger key={dataset.key} value={dataset.key} className="rounded-2xl px-4 py-2.5">
              {dataset.title[lang]}
            </TabsTrigger>
          ))}
        </TabsList>
