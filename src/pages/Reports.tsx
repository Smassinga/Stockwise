...
import { DOCUMENT_LOGO_PRINT_SIZE_PX, fitDocumentLogo, REPORT_PDF_LOGO_SIZE_MM } from '../lib/documentBranding'
...
export default function Reports() {
  ...
  async function exportPdf() {
    ...
    await runExport('pdf', async () => {
      ...
      const textX = logo ? 12 + REPORT_PDF_LOGO_SIZE_MM + 5 : 12
      if (logo) {
        const properties = doc.getImageProperties(logo.base64)
        const placement = fitDocumentLogo(properties.width, properties.height, {
          x: 12,
          y: 8,
          width: REPORT_PDF_LOGO_SIZE_MM,
          height: REPORT_PDF_LOGO_SIZE_MM,
        })
        doc.addImage(logo.base64, logo.extension === 'jpeg' ? 'JPEG' : logo.extension.toUpperCase(), placement.x, placement.y, placement.width, placement.height)
      }
