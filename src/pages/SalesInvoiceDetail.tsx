...
export default function SalesInvoiceDetailPage() {
  ...
  const [row, setRow] = useState<SalesInvoiceStateRow | null>(null)
  const [lines, setLines] = useState<SalesInvoiceLineRow[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (error) {
          if (isMissingFinanceViewError(error, SALES_INVOICE_STATE_VIEW)) {
            if (!cancelled) {
              setMissingView(true)
              setRow(null)
              setLines([])
        ...
        const lineRes = await supabase
          ...
          .select('id,sales_invoice_id,description,qty,unit_price,tax_rate,tax_amount,line_total,sort_order')
          .eq('company_id', companyId)
        ...
        if (!cancelled) {
          setRow(data as SalesInvoiceStateRow)
          setLines((lineRes.data || []) as SalesInvoiceLineRow[])
        }
      } catch (error: any) {
        console.error(error)
        if (!cancelled) {
          toast.error(error?.message || tt('financeDocs.salesInvoices.loadFailed', 'Failed to load sales invoices'))
          setRow(null)
          setLines([])
        }
      } finally {
  ...
  const orderLink = useMemo(() => {
    ...
    return `/orders?tab=sales&orderId=${encodeURIComponent(row.sales_order_id)}`
  }, [row])
        ...
        <>
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
            <Card className="border-border/80 shadow-sm">
              <CardHeader>
                <CardTitle>{row.internal_reference}</CardTitle>
              </CardHeader>
              <CardContent className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <div className="mt-1">{row.invoice_date}</div>
                </div>
                <div>
                  ...
                  <TableBody>
                    {lines.map((line) => (
                      <TableRow key={line.id}>
                        ...
                        <TableCell className="text-right font-mono tabular-nums">{formatDocumentMoney(line.unit_price, row.currency_code)}</TableCell>
