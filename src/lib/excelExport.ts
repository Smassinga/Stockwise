...
export async function exportExcelReport<T>(options: ExcelReportOptions<T>) {
  ...
  workbook.subject = [options.company.companyName, options.reportCode, options.reportingPeriod, options.displayCurrency]
    .filter(Boolean)
    .join(' | ')
  workbook.company = options.company.companyName
  ...
  const lastTableRow = Math.max(headerRowNumber, firstDataRow + options.rows.length - 1)
  sheet.pageSetup = {
    orientation: totalColumns > 7 ? 'landscape' : 'portrait',
    fitToPage: true,
    fitToWidth: 1,
    fitToHeight: 0,
    horizontalCentered: true,
    margins: { left: 0.35, right: 0.35, top: 0.55, bottom: 0.55, header: 0.2, footer: 0.25 },
    printArea: `A1:${sheet.getColumn(totalColumns).letter}${lastTableRow}`,
    printTitlesRow: `${headerRowNumber}:${headerRowNumber}`,
  }
  sheet.headerFooter = {
    oddHeader: `&L${options.company.companyName}&C${options.title}&R${options.reportingPeriod || ''}`,
    oddFooter: `&LGenerated by StockWise&C${options.company.footerNote || ''}&RPage &P of &N`,
    evenHeader: `&L${options.company.companyName}&C${options.title}&R${options.reportingPeriod || ''}`,
    evenFooter: `&LGenerated by StockWise&C${options.company.footerNote || ''}&RPage &P of &N`,
  }
