...
type PreviewMetadata = { semanticVariant: string; requiredFields: string[]; scenarioLabel: string }
...
export function EmailTemplateLab({ language }: { language: 'en' | 'pt' }) {
  ...
  async function render() {
    setBusy(true)
    try { const data = await invoke({ mode: 'preview', template_key: templateKey, language: previewLanguage }); setPreview(data.rendered); setPreviewMetadata(data.metadata) }
