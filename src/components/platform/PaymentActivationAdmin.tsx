...
export default function PaymentActivationAdmin({ locale, onOpenCompany, tt }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error(error)
      setLoadError(tt('platform.payment.queueUnavailable', 'Activation queue unavailable'))
  ...
  async function openRequest(request: PaymentRequest) {
    try {
      ...
      setSelected(detail.request); setEvents(detail.events); setReviewNote(detail.request.platform_review_note ?? ''); setActionResult(null)
  ...
  async function act(kind: 'review' | 'correction' | 'reject' | 'approve') {
    ...
    try {
      ...
      setActionResult(
        kind === 'approve'
          ? tt('platform.payment.approvalResult', 'Request approved. Updated entitlement evidence is shown above; confirmation email remains manual.')
          : tt('platform.payment.reviewResult', 'Review decision recorded in the immutable request history.'),
      )
  ...
  function openChannelEditor(channel: PaymentChannel) {
    setChannelDraft({
      ...emptyChannel,
      ...channel,
      account_name: channel.account_name ?? '',
      operator_instructions: channel.operator_instructions ?? '',
    })
    setChannelOpen(true)
  }
  ...
  const statusLabel = (value: string) => tt(`paymentActivation.status.${value}`, tt('administration.statusUnavailable', 'Status unavailable'))
  const eventLabel = (value: string) => tt(`paymentActivation.event.${value}`, tt('administration.eventUnavailable', 'Event unavailable'))
  const periodLabel = (value: string) => tt(`paymentActivation.period.${value}`, tt('administration.periodUnavailable', 'Period unavailable'))
  const providerLabel = (value: string) => tt(`paymentActivation.provider.${value}`, tt('administration.providerUnavailable', 'Provider unavailable'))
  const accessStatusLabel = (value?: string) =>
    value && isKnownSubscriptionStatus(value)
      ? tt(subscriptionStatusKey(value), tt('administration.statusUnavailable', 'Status unavailable'))
      : tt('administration.statusUnavailable', 'Status unavailable')
