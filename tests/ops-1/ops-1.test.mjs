...
test('POS stock cards use the maintained code-first EN or PT money formatter', () => { assert.match(operatorPage, /formatMoneyBase\([\s\S]*?row\.item\.unitPrice[\s\S]*?lang === 'pt' \? 'pt-MZ' : 'en-MZ'/); assert.doesNotMatch(operatorPage, /round2\(row\.item\.unitPrice \?\? 0\)\.toLocaleString/) })
