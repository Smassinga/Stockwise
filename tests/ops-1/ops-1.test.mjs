...
test('report-ready email uses its reporting period instead of a synthetic due date', () => { assert.match(templates, /key === "report_ready" && input\.period/); assert.match(templates, /key !== "report_ready"/); assert.match(templates, /key === "report_ready" \|\| key\.startsWith\("due_reminder_"\) \? 2 : 1/) })
test('email money groups four-digit MZN amounts in both maintained languages', () => { assert.match(emailMoney, /language === "pt" \? "pt-BR" : "en-US"/); assert.match(emailMoney, /useGrouping:\s*true/); assert.match(templates, /key\.startsWith\("due_reminder_"\) \? 2 : 1/) })
