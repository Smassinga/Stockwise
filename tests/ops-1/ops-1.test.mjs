...
test('email money groups four-digit MZN amounts in both maintained languages', () => { assert.match(emailMoney, /language === "pt" \? "pt-BR" : "en-US"/); assert.match(emailMoney, /useGrouping:\s*true/); assert.match(templates, /due_reminder_sales_order: 3/); assert.match(templates, /due_reminder_sales_invoice: 3/) })
