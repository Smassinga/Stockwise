...
const emailMoney = await read('supabase/functions/_shared/emailMoney.ts')
...
assert.match(templateLab, /operationalSales: 1250/, 'template lab uses numeric money evidence for localized rendering')
assert.match(templateLab, /missingCostEvidence: 1/, 'template lab digest scenario includes missing-cost evidence')
assert.match(templateLab, /topProductsServices: \[language === "pt" \? "Serviço QA" : "QA Service"\]/, 'template lab digest scenario includes top product and service evidence')
...
test('report-ready email uses its own reporting-period contract', () => { assert.match(templates, /type ReportReadyEmailInput/); assert.match(templates, /reportName: string; period: string/); assert.doesNotMatch(templates.match(/function renderReport[\s\S]*?function renderDigest/)?.[0] || '', /dueDate|outstandingAmount/) })
test('email money groups four-digit MZN amounts in both maintained languages', () => { assert.match(emailMoney, /language === "pt" \? "pt-BR" : "en-US"/); assert.match(emailMoney, /useGrouping:\s*true/); assert.match(templates, /due_reminder_sales_order: 4/); assert.match(templates, /due_reminder_sales_invoice: 4/) })
test('shared email layout provides the restrained QA banner and plain fallback', () => { assert.match(emailLayout, /This is a StockWise test email/); assert.match(emailLayout, /Este é um email de teste do StockWise/); assert.match(emailLayout, /TEST EMAIL/); assert.doesNotMatch(emailLayout, /#fef3c7|#78350f/) })
...
test('template lab records each provider acceptance exactly once through the shared mailer', () => { assert.match(templateLab, /templateVersion:\s*rendered\.templateVersion,[\s\S]{0,180}language,[\s\S]{0,180}qa:\s*true/); assert.doesNotMatch(templateLab, /admin\.rpc\(["']record_mail_dispatch_event/); assert.equal((templateLab.match(/sendTransactionalEmail\(/g) || []).length, 1) })
