...
- Hosted production has 28 active migrations and is aligned through `20260619175129_add_growth_batch_lifecycle_events.sql`. Growth Batches G1-G2 is live after the approved database-first rollout and controlled production smoke.
...
| Growth Batches | No | Company-scoped Growth Batch read policies; mutation is RPC-only; FORCE RLS on business tables | draft/activate/cancel/measurement/direct-cost RPCs | No | No | Request-key idempotency for create, activate, cancel, measurement, and direct cost; OPERATOR+ mutation; validation triggers; no stock/finance posting | Live G1-G2 package as of 2026-06-20. Authenticated clients can read permitted rows/views but cannot directly mutate Growth Batch business tables. Memo direct costs update Growth Batch rollups only and create no stock, COGS, finance, settlement, bill, journal, invoice, or `items.unit_price` changes. Controlled production smoke retained active batch `LEN-GB000000001` and verified unchanged stock, finance, and commercial price anchors. |
