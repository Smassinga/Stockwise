...
- Hosted production has 30 active migrations and is aligned through `20260620132656_add_growth_batch_stock_input_posting.sql`. Growth Batches G3 is live after the approved 2026-06-22 database-first rollout and controlled production smoke. The local repository now has 32 active migrations after the G4.1 mortality/shrinkage package, but those two migrations are not hosted/live until a future controlled rollout. The G3 rollout used commit `58e8a083c29d70d3b72aa755a80336393bcbb268`, Vercel deployment `dpl_CPHfKuoWcZ1eEMLrFXjv3cSFCu3i`, and GitHub Validation run `27930016751`.
...
| Growth Batches | No | Company-scoped Growth Batch read policies; mutation is RPC-only; FORCE RLS on business, G3 detail tables, and local G4.1 loss tables | draft/activate/cancel/measurement/direct-cost/preview-stock-input/post-stock-input/reverse-stock-input RPCs; local-only preview/record/reverse loss RPCs | No | No | Request-key idempotency for create, activate, cancel, measurement, direct cost, G3 stock input/reversal, and local G4.1 mortality/shrinkage/reversal; OPERATOR+ posting/recording; MANAGER+ reversal; validation triggers | Live G3 package as of 2026-06-22. Authenticated clients can read permitted rows/views but cannot directly mutate Growth Batch business tables. Memo direct costs remain non-financial; G3 stock inputs add physical issue movements and compensating receipt movements, freeze source WAC as material cost, keep original movements immutable, and create no finance, supplier liability, settlement, invoice, or commercial price change. Local G4.1 losses reduce only current quantity/latest weight, create no stock/finance/cost/price mutation, and are not hosted/live. |
