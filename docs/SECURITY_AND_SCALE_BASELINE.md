...
- Hosted production has 28 active migrations and is aligned through `20260619175129_add_growth_batch_lifecycle_events.sql`. Growth Batches G1-G2 is live after the approved database-first rollout and controlled production smoke. The local Growth Batches G3 stock-input package adds two pending migrations for a 30-migration local chain, has passed local replay, Growth Batches regression `5/5`, complete finance regression `31/31`, independent inspection, authenticated local visual QA, static validation, and build, and is not hosted or live until a separate database-first rollout succeeds.
...
| Growth Batches | No | Company-scoped Growth Batch read policies; mutation is RPC-only; FORCE RLS on business tables and new local G3 detail tables | draft/activate/cancel/measurement/direct-cost RPCs; local G3 preview/post/reverse RPCs pending hosted rollout | No | No | Request-key idempotency for create, activate, cancel, measurement, direct cost, and local G3 stock input/reversal; OPERATOR+ posting; MANAGER+ stock-input reversal; validation triggers | Live G1-G2 package as of 2026-06-20. Authenticated clients can read permitted rows/views but cannot directly mutate Growth Batch business tables. Memo direct costs update Growth Batch rollups only and create no stock, COGS, finance, settlement, bill, journal, invoice, or `items.unit_price` changes. Local G3 adds physical stock issue movements and compensating receipt movements for stock-input posting/reversal, freezes source WAC as material cost, keeps original movements immutable, and still creates no finance, supplier liability, settlement, invoice, or commercial price change. |
