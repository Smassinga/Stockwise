...
Growth Batches G3, G4.1, and G4.2 are live and production-smoke validated at `/growth-batches`. G4.1 extends the same workspace with mortality/shrinkage preview, recording, loss history, and MANAGER+ event-specific loss reversal; G4.2 adds a transfer preview/post/reversal surface. G5.1 adds a local-only Harvests tab for governed depleting harvest preview/posting and MANAGER+ event-specific harvest reversal. Hosted production remains on G4.2 until a future authorised rollout applies the G5.1 migrations. The workspace uses the same premium register/detail approach:

- `PremiumRegisterHeader`, metric cards, search/filter controls, desktop `PremiumDataTable`, and Android `PremiumMobileCardList`
- detail tabs for overview, stock inputs, G4.2 transfers, local G5.1 harvests, G4.1 losses, measurements, memo direct costs, timeline, and audit
- draft actions only while status is `draft`; measurement, memo direct-cost, G3 stock-input, G4.1 loss, G4.2 transfer, and local G5.1 harvest actions only while status is `active`
...
- local G5.1 harvest reversal is event-specific, MANAGER+ only, requires a reason, restores the original frozen quantity/weight/cost allocation when the harvested output remains available, and must not be labelled as sale reversal, COGS reversal, or whole-batch reversal
