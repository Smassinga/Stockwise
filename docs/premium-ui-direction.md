...
- detail tabs for overview, stock inputs, G4.2 transfers, G4.1 losses, measurements, memo direct costs, timeline, and audit
- draft actions only while status is `draft`; measurement, memo direct-cost, G3 stock-input, G4.1 loss, and G4.2 transfer actions only while status is `active`
