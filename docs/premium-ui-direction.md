...
The landing hero reserves the complete `max-w-4xl` reading region for the eyebrow, headline, explanation, actions, activation note, and supporting metrics. Decorative evidence cards are hidden below `1440px`; at and above that breakpoint they are fixed to `15rem` and positioned with calculated outer gutters, so language length, viewport width, and effective zoom cannot place them over the principal message. The cards remain non-interactive, hidden from assistive technology, and covered by the maintained reduced-motion rule.
