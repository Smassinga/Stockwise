...
| UXF-07 | P2 | `/settlements`, `/cash`, `/banks`, document detail | finance-anchor and correction logic is accurate but cognitively demanding | route and maintained copy review | corrected | UX-6 |
| UXF-08 | P2 | `/production-runs`, `/growth-batches` | dense histories/tabs and mixed terminology slow scanning | production PT navigation and route review | corrected | UX-7 |
...
| UX-8 Settings, users, Platform Control, and compliance | `/settings`, `/users`, `/platform-control`, `/compliance/mz` | setup map, role/status panels, admin registers | membership authority, subscription/access control, fiscal rules | UX-1/UX-3 | role matrix, EN/PT, responsive/admin QA completed | high | no | required and passed | no |
...
## 27. UX-4 Inventory Workspaces - Local Implementation

The UX-4 baseline at `a25ea977bd22f43a535be057dd2de316d96f3449` had 45 migrations and four inventory routes. The authenticated shell supplied 16px phone and 28px tablet/desktop clearance, but that rule was implicit. Items placed the full create form before its register, Movements placed governed posting before history, Stock Levels summed mixed base units and combined zero/negative risk, and Warehouses treated a failed bin read as an empty bin collection.

The local implementation formalizes shell-owned page rhythm at 16px phone, 20px tablet, and 24px desktop. Items defaults to `?view=register` and uses `?view=create` for focused creation. Stock Levels is read-only, retains WAC valuation, shows base UoM and available quantity, removes the mixed-unit total, and separates negative, out, low, healthy, and unconfigured-threshold evidence. Movements defaults to `?view=history`; `?view=record&type=...` keeps governed RPC posting focused, role-aware, idempotent, and double-submit guarded. Warehouses adds premium register state, status filtering, dataset-specific location import, and explicit partial bin-read evidence.

No route path, role, permission, RLS policy, migration, stock authority, costing rule, POS behavior, dependency, Sentry setting, or workflow trigger changed in UX-4.

Implementation `8c06d99b5caccdf4f2f265cd74f98ea467197a36` passed Validation run `29873186851`, job `88777795710`. Finance Regression (Isolated) run `29873250741`, job `88778008139`, replayed 45 migrations through `20260716130533`, passed `393/393`, cleaned up the ephemeral loopback stack, and uploaded no success artifact. Production deployment `dpl_AYHpdipMBT6Casqg8jWR2mdMpYJC` reached Ready for the implementation commit and served both `stockwiseapp.com` and the canonical redirect from `www.stockwiseapp.com`.

Read-only production QA covered Items, Stock Levels, Stock Movements, and Warehouses at measured widths `1442`, `1201`, `820`, and `390`, representing the required `1440`, `1200`, `820`, and `390` breakpoints. Portuguese dark-mode and English light-mode checks verified 24px desktop, 20px tablet, and 16px phone shell-owned top clearance, no double spacing, no horizontal overflow, no raw UUID fallback, no fallback UI, and zero console, React, or CSP diagnostics. Representative UX-3 routes (`/settings?view=setup`, `/setup/import`, `/users`, `/users/roles`, `/compliance/mz`, and the active-member `/onboarding` redirect) retained the same shared rhythm at desktop and phone widths. Production inspection was strictly read-only: no item, warehouse, bin, movement, import, POS-mode, or other business mutation occurred.

UXF-10 and UXF-15 are closed. The Page Rhythm Contract is durable for UX-5 through UX-10: authenticated page packages must retain shell-owned 16/20/24px first-content clearance across success, loading, error, empty, deep-link, and mobile states before adding page-local spacing. UX-5 Sales and Purchasing Workflows remains next; UX-6 through UX-10 retain their documented order.

## 28. UX-5 Sales And Purchasing Workflows - Local Implementation

UX-5 started from `8207e37d91b43145e65842f0c74537d60677ef7a` with 45 migrations through `20260716130533_add_pos_tax_applicability_mode.sql`, a clean tree, and `393/393` as the maintained finance-regression expectation. The initial `/orders` workspace exposed the correct governed operations, but the register mixed several row actions, mobile users received desktop tables, foreign-currency purchase orders had no configured-rate lookup, and failed or missing sales-rate reads could collapse into an unsafe-looking `1:1` value. Sales Invoice and Vendor Bill registers did not use the shared premium register system, while document details expressed workflow, approval, financial anchor, and settlement evidence in separate areas rather than one legible lifecycle.

The local implementation preserves the existing routes and adds a durable query-backed workspace contract: `tab=sales|purchase`, `view=register|create|detail`, and the existing `orderId`. Old links containing only `tab` or `orderId` remain valid. Sales and Purchase tabs reset to their register when switched, create opens the existing sheet, and selecting an order writes the detail state without changing the route path.

The commercial lifecycle is now presented as four distinct dimensions rather than one status: order/document workflow, stock fulfilment or receipt, finance-document state, and settlement. Sales Orders remain operational/commercial; an issued Sales Invoice becomes the legal fiscal and accounts-receivable anchor. Purchase Orders remain operational; receipt is independent from billing, and a posted Vendor Bill becomes the accounts-payable anchor. Existing one-bill-per-Purchase-Order handling remains authoritative and opens an existing bill rather than implying that a duplicate can be created.

The Sales Invoice and Vendor Bill registers now use canonical finance-document state views, shared premium headers, metric summaries, search and workflow/approval filters, one principal row action, contained desktop tables, and card-first mobile lists. Their amount hierarchy distinguishes document total, current legal amount, and outstanding amount. Supplier invoice reference remains separate from the StockWise internal Vendor Bill reference. If the canonical read fails, the register shows unavailable evidence rather than a false zero or migration instruction.
...
At UX-5 closure, UXF-16 was closed and UXF-07 remained assigned to UX-6 for deeper Settlement, Cash, Bank, and document-reconciliation work. UX-6 inherited the Page Rhythm and commercial lifecycle/anchor contracts.

## 30. UX-6 Finance, Reconciliation, And Remittance - Implementation

UX-6 started from `798b8065e0b593c125e7d3b827c2b52203880d6a` with 45 migrations through `20260716130533_add_pos_tax_applicability_mode.sql`, a clean tree, and `393/393` as the maintained finance-regression expectation. The initial Settlements page combined open exposure and reconciliation without a durable posted-activity register. Cash and Bank reads could collapse failures into valid-looking zero evidence. Bank Detail did not require a visibly selected statement before comparison, and the product had no shared company/counterparty finance-output contract.

Implementation `615dd19d889c2d03efb2e6429f0e726c31fd560b` preserves every finance RPC, active-anchor transition, two-decimal rule, idempotency key, role guard, reconciliation view, and company boundary. `/settlements` now whitelists `view=exposure|activity|reconciliation` and `side=ar|ap`; `/cash` uses `view=ledger|adjustment|settings`; `/banks/:bankId` uses `view=ledger|reconciliation|statements|import|settings`. Invalid values fall back safely, Back/direct links remain valid, and no query state mutates data.

Exposure presents the active operational or finance-document anchor, current legal amount, settled amount, outstanding amount, due/aging evidence, and one principal action. Settlement Activity uses a bounded 30-day default and preserves cash/bank evidence after exposure is resolved. Reconciliation consumes `v_finance_reconciliation_review` and `v_finance_reconciliation_exceptions` directly. Cash summary, Cash ledger, Bank register, Bank balances, statements, transactions, and book balance now retain independent loading, partial, unavailable, and stale-evidence states; a failed read is never represented as zero.

The shared finance-output layer supports Excel, PDF, and Print for single-event AP Remittance Advice, AR Receipt Allocation Advice, current filtered AR/AP exposure, settlement activity, reconciliation and exception views, Cash Book, Bank Ledger, and a selected Bank Reconciliation. External advice resolves company and counterparty identity, prefers immutable document snapshot evidence, falls back to company-scoped master data, masks bank identifiers, excludes internal notes, and includes the non-bank-proof disclaimer. Excel and PDF use the same numeric source model and WiseCore dark-teal/neutral styling. Optional grouped multi-event advice and a broader bounded counterparty-activity section are deferred because the current release does not need them to provide truthful single-event and filtered-view outputs.

## 31. UX-6 Production Rollout

Implementation Validation run `30210176169`, job `89814949978`, passed. The final guarded local run targeted `http://127.0.0.1:54321` and passed `393/393` in 97 seconds. Finance Regression (Isolated) run `30210229468`, job `89815088654`, used CLI `2.109.1`, replayed 45 migrations through `20260716130533`, passed `393/393`, cleaned up the ephemeral loopback stack, and uploaded no success artifact.

Production deployment `dpl_3w75AhC5uUm2tzbNfKe2kGAQTu2z` reached Ready for `615dd19d889c2d03efb2e6429f0e726c31fd560b` and served `stockwiseapp.com` plus the canonical redirect from `www.stockwiseapp.com`. Read-only QA covered Exposure, Settlement Activity, Reconciliation, Cash Book, Bank Accounts, Bank Detail, explicit no-statement reconciliation, and the receipt-advice confirmation surface at `1440`, `1200`, `820`, and `390`. Representative English/light and Portuguese/dark states had zero page overflow, mojibake, raw UUID/enum, fallback, console, React, or CSP findings. No report was downloaded from production and no settlement, adjustment, reconciliation, statement, import, bank-account, or POS-mode mutation occurred.

UXF-07 is closed. The next package is UX-7 Recipes, Production Runs, and Growth Batches, inheriting Page Rhythm, commercial lifecycle clarity, finance-anchor clarity, shared state behavior, and the finance export design rules where relevant.

## 32. UX-7 Recipes, Production Runs, And Growth Batches - Implementation

UX-7 started from `8dd371e2adfd7b0d17aa56ea800b1e1435468599` with 45 migrations through `20260716130533_add_pos_tax_applicability_mode.sql`, a clean tree, and `393/393` as the maintained finance-regression expectation. The initial Recipe route mixed creation, maintenance, planning, and assembly in one long surface. Production Runs kept create controls above the register and duplicated page padding. Growth Batch detail exposed ten equal-weight tabs and up to seven simultaneous active-batch actions. Production and Growth Batch surfaces also retained broad hard-coded English and mixed estimated, frozen, memo, harvested, and remaining cost language.

The production truth map retained the database as authority: Recipe identity/components/planning remain current editable definitions; Quick Assembly remains an idempotent append-only `BUILD` transformation; Production Run posted quantities and costs remain frozen immutable evidence; Growth Batch quantity, weight, location, costs, event sequence, and reversals remain canonical read-model evidence. No frontend calculation became stock, costing, lifecycle, or finance authority.

The workflow-selection map now presents Quick Assembly for simple Recipe-driven transformation, Production Run for planned-versus-actual and frozen-cost evidence, and Growth Batch for group-level biological or agricultural change over time. Recipes, Production Runs, and Growth Batches are register-first with whitelisted company-scoped query state. Growth Batch detail is reduced to Overview, Materials & Location, Lifecycle, Measurements, Costs, and History & Audit, while every maintained event and event-specific reversal remains reachable.

Recipe Specification, Production Run Cost Sheet, and Growth Batch Activity & Cost Report share one company-scoped model across Excel, PDF, and Print. Local controlled output checks covered numeric Excel cells, frozen headers, print setup, A4 PDF layout, company identity, currency evidence, row counts, disclaimers, long labels, Portuguese accents, missing optional company fields, and safe filenames. Generated QA files and fixtures were removed.

## 33. UX-7 Production Rollout

Implementation `ca2a8a98eada0d85ccdf5a7a5539572a7dd1b196` passed Validation `30218427150` (job `89836501935`). Read-only production QA found one enum-shaped completion summary; correction `b5e776070f7caa5c83f89456b51052adbe2c8098` localized that fallback and passed Validation `30219123263` (job `89838371997`). Final isolated run `30219161324` (job `89838480647`) used CLI `2.109.1`, replayed 45 migrations through `20260716130533`, passed `393/393`, cleaned up, and produced no success artifact.

Vercel deployment `dpl_GicBbkfvUNHX3bS95CR79NSf7cf2` served correction `b5e7760` as Production/Ready on `stockwiseapp.com` and `www.stockwiseapp.com`; source-map upload succeeded. Read-only QA covered the three registers, Recipe detail, Quick Assembly entry, Production Run detail, Growth Batch detail and all six sections across `1440x900`, `1200x800`, `820x1180`, and `390x844`, light/dark, and EN/PT. Page Rhythm, one-heading behavior, action density, report controls, cost language, raw-code/UUID absence, overflow, console, React, CSP, and asset checks passed. Production POS mode remained `non_fiscal`; no production Recipe, assembly, run, batch, event, report download, or database mutation occurred.

UXF-08 is closed. UX-8 Settings, Users, Platform Control, and Mozambique Compliance remains next. It inherits Page Rhythm, commercial and finance-anchor clarity, finance export standards, and the production terminology/state contracts established here.

## UX-8 Administration And Compliance Rollout

UX-8 started from `a34fdb266c64145762a6ff2a85e5fe1b7b49bad0` with 45 migrations through `20260716130533_add_pos_tax_applicability_mode.sql` and `393/393` as the maintained regression expectation. Settings was a dense mixed-purpose surface, Platform Control was a single long operating page, Compliance mixed readiness and optional history, and invitation outcomes did not make token creation versus email delivery sufficiently durable.

Implementation `fb7fbb1fb42abea3f82767dda9a7a64fc51ee52f` introduced query-backed administration workspaces without changing authority. Production review found Platform Control Portuguese leakage; localization recovery `5c7cd6cb3a953c9df1177658046e7ea417fcc85b` corrected it. The first browser harness rejected protected-route navigation before authenticated visual evidence could be collected; that was an environment/tool limitation, not a demonstrated StockWise defect. Release remained blocked when the authorised production testing company was expired. Leny Doçuras was then legitimately reactivated as the authorised authenticated testing company, not as an artificial route-guard bypass.

Supported authenticated Chrome/Playwright QA confirmed Leny Doçuras as an allowed trial company, the testing account as company Admin and independently platform admin, and no Company Access redirect. Read-only production checks covered Settings, Users, Roles, Platform Control, and Mozambique Compliance at `1440x900`, `1200x800`, `820x1180`, and the host-scaled `390x843/845` bracket around the requested `390x844`, across English/Portuguese and light/dark. Settings retained local saves, authority ownership, weighted-average-only valuation, truthful communications support, and platform-managed access. Users retained active/invited/disabled evidence and role protections. Compliance retained core-versus-optional evidence, localized series/audit labels, and the explicit non-SAF-T workbook boundary.

Production QA found one genuine defect: closing member review did not return focus to the originating trigger. Correction `c8c135bf4f3dcfb2bea30f010be9b5bb165de674` restored focus for explicit close and Escape. Validation run `30386979350`, job `90368613907`, passed; isolated run `30387076169`, job `90368951757`, used CLI `2.109.1`, replayed 45 migrations, passed `393/393`, cleaned up, and produced no artifact. Deployment `dpl_3aEa6djVPVX98kKcPocDgppTbL1j` reached Production/Ready and the corrected focus behavior was reverified on `stockwiseapp.com`.

No production setting, member, invitation, activation, access, email, reset, fiscal, workbook, SAF-T, or POS mutation occurred. POS remained `non_fiscal`. No migration, RPC, view, Edge Function, dependency, package-lock, role, entitlement, reset, fiscal-authority, Sentry, CSP, or workflow-trigger change occurred. UX-8 is closed; UX-9 Accessibility, Localization, Motion, and Final Consistency remains next and was not started.

## UX-9A Conversion, Motion, Setup, And Currency Feedback

UX-9A started from `bed76a4c7e22b4865d38bbb2ad39a8cf74dc521f` with 45 migrations through `20260716130533_add_pos_tax_applicability_mode.sql`, a clean tree, and `393/393` as the maintained regression expectation. It is a controlled subset of UX-9 and does not begin the wider accessibility, localization, motion, or consistency audit.

Magic UI Interactive Hover Button, Pulsating Button, and Glare Hover sources and dry-run proposals were reviewed. No registry files or dependencies were accepted. Implementation `af3c4c2972eddd28a716773e11455e9e908bec74` added link-safe landing CTAs, one logged-out hero pulse, restrained pricing/final-CTA glare, the maintained demo mailto action, real-state onboarding feedback, a preview-only FX conversion model, and compact saved-rate evidence. Production review found one real defect at the Portuguese mobile breakpoint: the menu control retained an English accessible name. Correction `ae07e69408800bb80ff8cc6214fea83342a785eb` localized the open/close labels without changing navigation behavior.

Onboarding loader changes follow the existing account check, company bootstrap, active-company selection, membership polling, and invitation acceptance operations. Controlled local tests covered creation, timeout recovery, invitation success/failure, duplicate-submit prevention, retained values, and immediate durable completion. Currency tests covered both entered directions without inversion, configured decimals, incomplete and invalid inputs, same-currency and unavailable-metadata states, read-only/editable roles, save success/failure, and no false zero.

Normal/reduced motion, fine-pointer, touch/no-hover, keyboard focus, CTA semantics, loader announcements, preview announcements, EN/PT, and light/dark behavior were checked locally. The final loopback regression passed `393/393` in 107.97 seconds with controlled fixtures removed. Validation run `30408169941`, job `90438226210`, passed for the correction. Isolated run `30408248285`, job `90438464380`, used CLI `2.109.1`, replayed 45 migrations through `20260716130533`, passed `393/393`, cleaned up, and produced no success artifact.

Production deployment `dpl_4jZPzpsesu2LAv4MEdoYDxBffJ9B` reached Ready for `ae07e69408800bb80ff8cc6214fea83342a785eb`, served `stockwiseapp.com` and `www.stockwiseapp.com`, and uploaded source maps successfully. Read-only QA covered the landing page at `1440x900`, `1200x800`, `820x1180`, and `390x844`, logged-out and authenticated states, representative EN/PT and light/dark combinations, onboarding existing-session routing, and the authenticated Currency preview. There was no page overflow, pricing-card height regression, nested interactive control, clipped pulse/glare, false completion, false zero, translation key, mojibake, console error, React warning, or application CSP refusal after correction.

No production company, invitation, membership, currency, FX rate, base currency, pricing, activation, finance, or POS data changed. POS remained `non_fiscal`. No migration, RPC, view, Edge Function, dependency, package-lock, base Button/Card, pricing calculation, authority, route guard, Sentry configuration, CSP, or workflow trigger changed. UX-9A is closed independently; the remaining UX-9 audit and UX-10 were not started.
## UX-9B — Shell, workspace consistency, and landing conversion copy

UX-9B started from clean `main` at `d17d9673e7eaf169a653298afe888f78575c03eb`, 45 migrations through `20260716130533_add_pos_tax_applicability_mode.sql`, CLI marker `v2.101.0`, and the maintained `393/393` regression expectation. The route inventory covered registers; query-backed create/detail order states; invoice, bill, bank, production and batch details; mobile-card registers; desktop tables; and sticky action surfaces.

Implementation corrected independent desktop navigation scrolling, reinforced shared page widths and typography, compacted register toolbars, aligned Currency hierarchy/contrast, and corrected monetary Gross profit / Lucro bruto terminology without changing calculations. The landing now leads with business control, explains the cost of scattered records without fabricated proof, and presents a five-step guided implementation path while preserving the seven-day trial, four plans, manual activation, pricing, demo mailto, EN/PT, Magic UI-inspired CTAs, and reduced motion.

Local static gates passed after both implementation commits. The guarded loopback finance suite targeted `http://127.0.0.1:54321` and passed `393/393`; an initial stale local JWT harness failure occurred before assertions and the rerun used current process-local stack keys. Implementation Validation `30479673311` (job `90670156653`) passed for `d805ced86893f212b045a9e1a5ac7dcb4bcc254c`; deployment `dpl_HcCN6F6pPggw23UoTpoZzhjK9xu9` reached Production/Ready. Read-only production review used the existing Leny Doçuras session and found one machine-like register count, corrected to natural EN/PT wording without business mutation.

## UX-9C Owner Performance Cockpit

UX-9C replaces the long Dashboard analysis surface with an operational owner cockpit. The first row is fixed to operational sales, gross profit, gross margin, completion rate, and open orders; current inventory and exception indicators follow as secondary evidence. Period and warehouse scope are URL-backed, custom dates apply explicitly, daily detail remains available in a sheet, and the full-width latest-movements section is removed without removing maintained inventory navigation.

Implementation `97c8b2e0bc1b9b2a891b082970b82bf0d56316ae` passed exact Validation `30488865757`. The isolated workflow initially stopped at its stale 45-migration guard before executing tests; correction `e573c2a24cf6d7070053e80d845c6616b581bc22` updated only the expected migration count/latest version, passed Validation `30489224695`, and isolated run `30489288808` (job `90702845713`) then replayed 46 migrations through `20260729143000` and passed `393/393`. Dashboard tests passed `17/17`.
...
## UX-9D Landing Hero And Dashboard Trend Clarity

UX-9D began from clean `main` at `3f49d2166f6b75dc56de92b924a2eb72a1f38acd`, 48 migrations through `20260729214142_integrate_service_actuals_into_owner_dashboard.sql`, CLI marker `v2.101.0`, and the maintained finance `393/393`, Dashboard `17/17`, and Service Jobs `34/34` expectations. Inspection confirmed that headline and trend evidence come from the same `get_owner_dashboard` response, period sales are totals, trend rows are daily, service activity uses `actual_completion`, daily Gross profit exists when cost evidence is complete, and no backend reconciliation defect or QA-prefix exclusion exists.

Implementation `1453d3f6b07ddc0b3841e60fc70e5f2fff5c8a31` moved the decorative landing evidence cards from the unsafe `1024px` rule to a `1440px` outer-gutter contract, reduced them to `15rem`, and made the group assistive-technology decorative. The Dashboard no longer references undefined `--success`: Operational sales retains primary colour, COGS uses dashed `--chart-cogs-line`, and solid Gross profit uses `--chart-margin-line`. The chart distinguishes daily values from selected-period headline totals, and Daily details includes supported or explicitly unavailable Gross profit without converting null to zero.

Validation `30592376743` passed on the exact implementation commit. Isolated run `30592442221`, job `91037474431`, replayed 48 migrations, passed finance `393/393`, Dashboard `25/25`, and Service Jobs `34/34`, and destroyed its loopback stack. Deployment `dpl_J6nfeDZBa6s1uWRfcPKEm7qLjtJj` reached Production/Ready on the canonical public aliases.

Measured landing QA covered `1366x768`, `1440x900`, `1536x768`, `1920x1080`, and `2560x1440` in EN/PT, with effective smaller-width checks covering zoom-safe hiding and the maintained reduced-motion contract. The `896px` reading region remained centred with no card intersection or page overflow. Authenticated read-only Dashboard QA covered `1440x900`, `1366x768`, `1200x800`, `820x1180`, and `390x844`, EN/PT, and light/dark. All three lines, six normal dots, active tooltip state, legend, daily explanation, and Daily details were visible without overflow or legend clipping; console, React, and CSP findings were zero.

Production reconciliation remained MZN 1,020.00 sales, MZN 426.95 known COGS, and MZN 593.05 Gross profit across the July 16 and July 29 points; 2 transactions reconcile to MZN 510.00 average value and 58.1% displayed Gross margin. The existing service item remained present on July 29, its actual completion date. Production POS mode remained `non_fiscal`; no business row, Service Job, inventory, document, POS, period, warehouse, pricing, subscription, migration, RPC, or database function was changed. UX-10 was not started.
