...
| Document output language behavior | Not started | Frontend + compliance/output model | Locale/source-of-truth decision | `src/lib/mzInvoiceOutput.ts`, `src/lib/financeDocumentOutput.ts`, `src/lib/i18n.tsx`, `src/pages/Settings.tsx`, `src/pages/MozambiqueCompliance.tsx` | Maybe, if snapshot rules are refined | Yes | Issue/download in `pt` and `en`, compare output language | Current helpers still hardcode Portuguese labels and formatting even though language settings and snapshots exist |
