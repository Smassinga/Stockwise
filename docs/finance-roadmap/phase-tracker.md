...
| Adaptive shell and notification usefulness refinement | Completed | Responsive shell + notification signal quality | Wider screens now use space more intelligently, compact layouts keep the bottom dock clearer, and the notification feed now includes high-signal finance document approvals/issue-post milestones | Keep notification scope high-signal; do not turn it into broad activity spam |
...
| Repo current-truth and release-readiness pass | Completed | Docs + Tauri metadata + file audit | The repo now carries a smaller current-truth doc set, cleaner stale-file surface, and Tauri desktop/Android release notes aligned with the current product | Brochure-style docs, dead helper files, and stale bootstrap-era scripts were removed |
