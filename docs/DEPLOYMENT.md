...
## Commercial tax package release gate (local-only)

Hosted production remains at 41 migrations while local replay has 43. A release may propose only `20260712052825_add_commercial_tax_integrity.sql` and `20260712052833_add_item_profile_trust.sql`. Before hosted application, require a clean replay, static/build checks, the complete guarded finance regression, the 122 named package checks, responsive EN/PT QA, a clean Git diff, passing GitHub Validation, and a linked dry run showing exactly those two migrations.

The database must be deployed before exposing the matching frontend. Controlled smoke must use a QA company, synthetic tax labels/rates, draft SO/PO and draft SI/VB only, and one clearly labelled QA item. Do not issue a legal invoice or post a Vendor Bill. Restore original defaults and do not leave synthetic rates customer-facing. Production attack, concurrency, cross-company, immutable-document, and rounding-abuse tests remain local-only.
