...
# POS tax applicability regression (2026-07-16 release candidate)

The package adds 105 named Node test checks to the prior `288`: 32 configuration/schema/security checks in `commercial-tax-integrity.test.mjs`, plus one scenario group containing 72 state checks in `finance-regression.test.mjs`. Coverage distinguishes unconfigured rollback, configured standard/zero/exempt tax, required exemption reasons, non-fiscal cash and bank posting, immutable snapshots, idempotent replay and mismatch, fiscal-invoice prohibition, settled exposure, stock and price isolation, and Production Run/Growth Batch isolation. The expected complete suite is `393/393`; only the guarded loopback target `http://127.0.0.1:54321` is permitted.

The migration replay gate is 45 active migrations through `20260716130533_add_pos_tax_applicability_mode.sql`. Production mutation is not part of this coverage; hosted verification is catalog/read-only after the approved migration rollout.
