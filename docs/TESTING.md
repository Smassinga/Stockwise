...
- Growth Batches G3 local stock-input coverage, including preview/no-mutation behavior, base-UOM enforcement, insufficient-stock and duplicate-bucket blockers, physical issue movement references, frozen WAC material cost, rollups, replay/mismatch, MANAGER+ reversal, compensating receipt references, direct mutation rejection, and finance/price isolation
...
Growth Batches G3 is a local-only stock-input posting package pending hosted rollout. The local regression extension must run only against a confirmed local Supabase target. It covers `preview_growth_batch_stock_input`, `post_growth_batch_stock_input`, and `reverse_growth_batch_stock_input`; operation types `growth.batch.input` and `growth.batch.input.reverse`; physical issue/receipt movement references; base-UOM-only consumed lines; WAC-derived frozen material costs; material/direct/total/remaining rollups; request-key required/replay/mismatch behavior; MANAGER+ reversal authority; direct mutation rejection on stock-input and reversal detail tables; and no attributable cash, bank, vendor bill, invoice, finance document event, supplier liability, settlement, or `items.unit_price` mutation.

G3 local validation passed before Git finalisation: 30-migration replay passed, Growth Batches targeted regression passed `5/5`, complete finance regression passed `31/31`, independent implementation inspection passed, authenticated local visual QA passed at `1440`, `1200`, `820`, and `390` in light and dark mode, and static validation/build passed. The local-only visual QA used company `G3 Visual QA Local 20260621120349`, batch `G3 Visual Batch 20260621120349`, batch reference `GVI-GB000000001`, and stock-input event `GVI-GB000000001-E000002`. It verified valid preview, stale-preview protection, duplicate source-line rejection, insufficient-stock blocking, OPERATOR+ posting, frozen WAC `MZN 2.50`, posted material cost `MZN 12.50`, stock-input history, MANAGER+ event-specific reversal with mandatory reason, compensating receipt, preserved original issue movement, cost restoration to `MZN 0.00`, and stock restoration to `100 EA at MZN 2.50 WAC`.

G3 production replay, payload-mismatch, concurrency, and failure tests must not be performed in production. They belong in the guarded local finance regression suite and controlled local visual QA until a separate database-first rollout and production smoke plan is approved.
