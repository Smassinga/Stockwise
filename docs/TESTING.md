...
## Commercial tax integrity regression (local-only)

`commercial-tax-integrity.test.mjs` contains 122 numbered checks covering configuration authority/audit, no-default behavior, explicit zero/exempt treatment, mixed rates, discount-before-tax rounding, derived headers, post-confirmation locks, canonical SO-to-SI and PO-to-VB snapshot propagation, legacy fallback, item-profile round trips, client-helper privileges, localization alignment, canonical Sales Order display totals, finance-anchor state refresh after order status transitions, and stock/finance/price isolation. The test target guard must remain active and this mutation suite must run only against local or explicitly isolated non-production Supabase.

The complete maintained command remains `npm run test:finance-regression`. Growth Batch CLI-query checks may take longer when run after other mutation-heavy files; an isolated rerun may diagnose local CLI contention but does not replace the required complete command for release sign-off.
