...
The first Production Runs package is live and production-smoke validated as of 2026-06-18. Hosted Supabase is aligned through `20260615213640`, the production frontend is commit `4f82c5a`, and the local finance regression suite passed `26/26` before rollout. The controlled smoke used `Leny Doçuras`, recipe `Bolo Custarde` v1, and Production Run `LEN-PR000000001`. A setup stock receipt for `Fermento` was submitted once through the maintained Movements UI, moving `Casa / CDC001 - Cozinha - Casa` stock `0 -> 3` and creating succeeded `stock.receipt` request `9b1f5e7c-a046-458c-a889-6f4056d36805`. The Production Run post was submitted once through `/production-runs`, created succeeded `production.run.post` request `33facecd-a63e-45c9-939d-2179303031b1`, seven input issue movements, and one output receipt movement `bb2fe802-9d58-4f7a-9118-982a44ef84ce`; Fermento moved `3 -> 0` and `Bolo de Custarde` in `Casa / QA-A2 - A2 Production Smoke` moved `0 -> 1`. The immediate reversal was submitted once, created succeeded `production.run.reverse` request `54409ae2-d3f4-483f-8b7b-ecfd66717ae9`, one compensating output issue `2991d192-223d-42a3-b017-c41850d43c5b`, and seven compensating input receipts; Fermento returned `0 -> 3` and the QA output bucket returned `1 -> 0`. Duplicate stock buckets and negative stock rows remained zero, `items.unit_price` remained `1500`, and no cash, bank, or vendor-bill rows were created by the Production Run. No production replay or payload-mismatch test was performed. Replay, mismatch, authority, concurrency, failure-path, base-UOM, finance-isolation, and reconciliation paths remain covered by the local `26/26` suite. The Production Runs UI requires a fresh readiness preview after material draft changes and exact run-reference confirmation before reversal. The production-target guard in `tests/finance-regression/helpers.mjs` remains mandatory and must print a local safe target before mutation tests.

The Growth Batches G1-G2 package is local-only until hosted Supabase is explicitly approved and `npx supabase db push` succeeds in that session. Hosted production remains at 26 migrations through `20260615213640_add_production_run_posting.sql`; the local uncommitted Growth Batches branch has 28 active migrations. Validation must include `npm run check:migrations`, `npx supabase db reset --local --no-seed`, `npm run lint:js`, `npm run check:css-vars`, `npm run check:css-classes`, `npm run build`, `npm run test:finance-regression`, and `git diff --check`. The regression suite must cover Growth Batch RLS/FORCE RLS, RPC-only mutation, idempotent create/activate/cancel/measurement/direct-cost replay, payload mismatch rejection, null/zero/omitted hash distinctions, numeric hash equivalence, draft-only edit/cancel, active-only measurement/direct-cost entry, whole-number count quantities, batch-linked weight and area UOM validation, chronology guards, concurrent event sequencing, concurrent direct-cost rollups, deterministic read-model ordering by explicit caller `ORDER BY`, unchanged `stock_movements`, unchanged finance anchors, and unchanged `items.unit_price`.
