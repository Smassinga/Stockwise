...
The first Production Runs package is implemented locally and is not live until hosted migration rollout and frontend deployment. The finance regression suite now covers draft creation from an active BOM, seeded input scaling, draft update, draft cancellation without movement, non-mutating preview, insufficient-input readiness, estimated material/direct/total/unit cost, idempotent posting, same-key replay, payload mismatch, blank request-key rejection, OPERATOR/VIEWER/MANAGER authority split, RPC-only table mutation denial, cross-company rejection, base-UOM-only enforcement, frozen cost snapshots, unchanged `items.unit_price`, additional costs with no cash/bank/AP/vendor-bill/settlement/journal side effects, controlled compensating reversal, original movement immutability, replayed reversal, second reversal blocking, insufficient-output reversal rollback, insufficient-input posting rollback, ledger-to-rollup reconciliation, no duplicate stock buckets, no negative stock, and concurrent Production Run versus stock-issue competition. The Production Runs UI requires a fresh readiness preview after material draft changes and exact run-reference confirmation before reversal. The production-target guard in `tests/finance-regression/helpers.mjs` remains mandatory and must print a local safe target before mutation tests.
