...
## Commercial tax integrity regression

`commercial-tax-integrity.test.mjs` and the maintained finance regression contain 126 numbered package checks covering configuration authority/audit, no-default behavior, explicit zero/exempt treatment, mixed rates, discount-before-tax rounding, derived headers, post-confirmation locks, canonical SO-to-SI and PO-to-VB snapshot propagation, legacy fallback, item-profile round trips, client-helper privileges, localization alignment, canonical Sales Order display totals, corrected finance-anchor state, and stock/finance/price isolation. The test target guard must remain active and this mutation suite must run only against local or explicitly isolated non-production Supabase.

The complete maintained command remains `npm run test:finance-regression`. Final release validation passed `288/288` after a clean `44/44` replay. Production smoke covered only valid configured/no-default draft paths; attack, concurrency, cross-company, immutable-document, and rounding-abuse mutation tests remained local-only. Growth Batch CLI-query checks may take longer when run after other mutation-heavy files; an isolated rerun may diagnose local CLI contention but does not replace the required complete command for release sign-off.

## Isolated Finance Regression CI

`.github/workflows/finance-regression-isolated.yml` introduces a manual-only `workflow_dispatch` gate for the complete finance regression. It creates a fresh ephemeral local Supabase stack with CLI `2.109.1`, applies all 44 migrations, exports only local stack credentials for the job, and runs the same files serially through `npm run test:finance-regression:ci` without loading `.env`. The workflow hard-fails unless the API is exactly `http://127.0.0.1:54321`, retains the permanent production-project block, supplies no external mail/AI/webhook secrets, and always destroys the local stack. Failure-only diagnostics are sanitized and retained for three days. Initial execution evidence must be recorded before considering push, pull-request, or scheduled triggers.

The first successful manual run completed on 2026-07-15 as workflow `29453038854`, job `87479849879`. It used Supabase CLI `2.109.1`, replayed all 44 migrations through `20260712230118`, passed `288/288` against `http://127.0.0.1:54321` in `57.8s` of test execution (`4m10s` total job time), and completed `supabase stop --no-backup`. No production or hosted credential was supplied. Failure artifacts remain failure-only, sanitized, and retained for three days. The workflow remains manual; push, pull-request, and scheduled execution require a later cost/runtime decision.

## WiseCore palette rollout validation (2026-07-16)

The palette implementation passed `check:migrations` with 44 active migrations through `20260712230118_fix_canonical_sales_order_finance_state.sql`, JavaScript lint, CSS variable/class checks, a credential-free production build, and `git diff --check`. The guarded local finance regression passed `288/288` against `http://127.0.0.1:54321` in `112.2s`.

Implementation commit `f53bb2500dd336eb4967e90d35dd6a6d6ae8b9b6` passed Validation run `29455999390`, job `87489184718`. Manual Finance Regression (Isolated) run `29456064325`, job `87489388349`, then passed `288/288` with 44 locally replayed migrations and successful ephemeral cleanup.

Production visual QA on deployment `dpl_BG7cpLzLPYyp53Ys6Fngf5LE9gk4` covered `1440`, `1200`, `820`, and `390`, both themes, public auth surfaces, the landing page, and the approved authenticated operational and finance routes. Representative Portuguese checks passed. Results were zero page-level overflow, zero raw package backend codes, zero browser console errors, and zero CSP errors. No business record was created or changed for visual validation.
# POS tax applicability regression (2026-07-16 live rollout)

The package adds 105 named Node test checks to the prior `288`: 32 configuration/schema/security checks in `commercial-tax-integrity.test.mjs`, plus one scenario group containing 72 state checks in `finance-regression.test.mjs`. Coverage distinguishes unconfigured rollback, configured standard/zero/exempt tax, required exemption reasons, non-fiscal cash and bank posting, immutable snapshots, idempotent replay and mismatch, fiscal-invoice prohibition, settled exposure, stock and price isolation, and Production Run/Growth Batch isolation. The complete guarded local suite passed `393/393` against `http://127.0.0.1:54321` in `97.627s` of Node test time.

The migration replay gate is 45 active migrations through `20260716130533_add_pos_tax_applicability_mode.sql`. Validation run `29515956021`, job `87680918687`, passed for implementation `f559f253c70823786c2ebd388bab3b26c02262b7`. The first isolated run correctly failed its stale 44-migration infrastructure assertion before tests; compatibility commit `046d4500ed58e57711460f0d517a1995bf6cbe4f` updated only the expected migration count/latest value and passed Validation `29516389860`, job `87682378112`. Isolated run `29516469375`, job `87682655940`, then replayed 45 migrations, passed `393/393` in `59.953s`, and cleaned up the ephemeral loopback stack without a success artifact. Production mutation is not part of this coverage; hosted verification remained catalog/read-only after rollout.

## SVC-1 verification

Validation `30495374076` passed the two implementation commits at `2ea2d2768d7d84bfd43164d46c8604bce5216fb1`. The first isolated run `30495451534` encountered a non-2xx cold-start response from the pre-existing mail preview Edge Function; unchanged rerun `30495821761` replayed 48 migrations through `20260729214142` and passed finance `393/393`, dashboard `17/17`, and Service Jobs `34/34`.

Production-smoke corrections `63b67a9` and `7a8b2e6` made customer-material UoMs canonical and rendered their readable codes. Exact Validation runs `30496973546` and `30497408465` passed.
...
`npm run test:ops-1` covers default-active controls, RPC/role boundaries, immutable events, promise evaluation, reminder suppression and final recheck, anchor movement, settlement closure, EN/PT UI, reporting, notification targeting, and protected Template Lab suppression. Migration validation includes a reset from zero and security/grant inspection.
