...
- Growth Batches G3 is implemented locally, pending hosted rollout, with a card-first stock-input workflow for active batches. Authenticated local visual QA passed at `1440`, `1200`, `820`, and `390` in light and dark mode, including multi-line item/source/quantity entry, preview blockers, WAC/material-cost estimates, final post action, stock-input history, and MANAGER+ reversal controls without page-level horizontal scrolling. G4/G5 actions such as mortality, shrinkage, transfers, harvest, completion, whole-batch reversal, FIFO, COGS, fair value, and finance posting remain unavailable.
...
- growth-batch cards and detail sections should show status, family, basis quantity, latest weight, memo/direct/material costs, location, measurements, stock-input history, and event timeline without requiring horizontal scrolling
