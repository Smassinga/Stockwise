drop function if exists public.prepare_sales_invoice_for_issue_mz(uuid);

create or replace function public.prepare_sales_invoice_for_issue_mz(
  p_invoice_id uuid,
  p_vat_exemption_reason_text text default null
)
...
comment on function public.prepare_sales_invoice_for_issue_mz(uuid, text) is
  'Backfills issue-time legal snapshots, exemption wording, and base totals for a draft sales invoice through a narrow, controlled path before issue.';

revoke all on function public.prepare_sales_invoice_for_issue_mz(uuid, text) from public, anon;
grant execute on function public.prepare_sales_invoice_for_issue_mz(uuid, text) to authenticated;
